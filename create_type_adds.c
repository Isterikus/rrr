...
/*   Updated: 2017/05/12 13:36:24 by dkovalen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "header.h"

int			flags(const char *type, int i, t_printf *elem)
{
	int		j;
		...
		type[i] == ' ' || type[i] == '#' || type[i] == '0')
	{
		...
		type[i + j] == ' ' || type[i + j] == '#' || type[i + j] == '0')
			j++;
		elem->flags = ft_strjoin3(elem->flags, ft_strsub((char *)type, i, j));
		i += j;
	}
	return (i);
}
...
int			int_w(const char *type, int i, t_printf *elem, va_list *ap)
{
	if (type[i] == '*')
	{
		...
		if (elem->width < 0)
		{
			elem->flags = ft_strjoin2(elem->flags, "-");
